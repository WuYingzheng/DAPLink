...
typedef struct cfg_ram {
    uint32_t key;                    // Magic key to indicate a valid record
    uint32_t size           : 16;    // Offset of the last member from the start
    uint32_t hold_in_bl     : 8;     // Configurable values
    uint32_t reserved0      : 8;

    char assert_file_name[64];

    uint32_t assert_line    : 16;
    uint32_t assert_source  : 8;
    uint32_t valid_dumps    : 8;        // Additional debug information on faults
...
// Configuration RAM
static cfg_ram_t __attribute__((section("cfgram"))) config_ram = {0};
